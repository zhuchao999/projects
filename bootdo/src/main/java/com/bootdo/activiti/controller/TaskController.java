...
public class TaskController {
    ...
    List<TaskVO> todoList(){
    	...
    	if(tasks==null || tasks.isEmpty()){
    		//如果没有，则按角色查
    		String uname = ShiroUtils.getUser().getUsername();
        	List<String> candidateGroups  = new ArrayList<String>();
        	if (uname.equals("wjl")) {
        		candidateGroups.add("人力资源部长");
    		}
        	if (uname.equals("gdg")) {
        		candidateGroups.add("薪酬主管");
    		}
        	tasks = taskService.createTaskQuery().taskCandidateGroupIn(candidateGroups).list();
    	}
